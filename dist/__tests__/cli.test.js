...
describe("CLI: add", () => {
    ...
    it("adds a task with tags and deps", () => {
        ...
        const result = runCli(["add", "Dependent task", "--tags", "research,health", "--deps", "task-001"], storePath());
        expect(result.ok).toBe(true);
        const store = JSON.parse(fs.readFileSync(storePath(), "utf-8"));
    ...
    it("adds a task with description", () => {
        const result = runCli(["add", "Task with desc", "--description", "This is a detailed description"], storePath());
        expect(result.ok).toBe(true);
        const store = JSON.parse(fs.readFileSync(storePath(), "utf-8"));

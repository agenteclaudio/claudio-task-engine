...
function renderCard(task, store) {
...
function categorize(tasks, store) {
...
function renderStats(store, columns) {
...
export function generateKanban(store) {
    const columns = categorize(store.tasks, store);
    const allTags = [...new Set(store.tasks.flatMap((t) => t.tags))].sort();
    const allExecModes = [...new Set(store.tasks.map((t) => t.execution_mode).filter(Boolean))].sort();
    const allTaskTypes = [...new Set(store.tasks.map((t) => t.task_type).filter(Boolean))].sort();

...
import { TaskStoreSchema, ok, err } from "./types.js";
export function loadStore(filePath) {
    try {
        ...
        return err(`Failed to load store: ${e.message}`);
    }
}
export function saveStore(filePath, store) {
    try {
        ...
        return err(`Failed to save store: ${e.message}`);
    }
}
function nextId(tasks) {
    if (tasks.length === 0)
        return "task-001";
    const maxNum = Math.max(...tasks.map((t) => {
        const m = t.id.match(/^task-(\d+)$/);
        return m ? parseInt(m[1], 10) : 0;
    }));
    return `task-${String(maxNum + 1).padStart(3, "0")}`;
}
function wouldCreateCycle(tasks, newTaskId, deps) {
    const allTasks = [...tasks, { id: newTaskId, dependencies: deps }];
    const adj = new Map();
...
export function addTask(store, input) {

...
export function computeStatus(tasks: Task[]): Task[] {
...
export function computeWaves(tasks: Task[]): Wave[] {
  ...
  const remaining = tasks.filter(
    (t) => t.status !== "completed" && t.status !== "failed" && t.status !== "in_progress"
  );
  ...
  while (processed.size < remaining.length) {
    ...
    if (waveTasks.length === 0) break; // Remaining tasks have cycles or unmet deps

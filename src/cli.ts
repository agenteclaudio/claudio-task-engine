...
import { loadStore, saveStore, addTask, updateTask, deleteTask, type AddTaskInput, type UpdateTaskInput } from "./store.js";
...
const DEFAULT_STORE_PATH = process.env["CTE_STORE"] ?? path.join(
...
export function runCli(args: string[], storePath?: string): Result<string> {
...
function cmdAdd(positional: string[], flags: Record<string, string>, sp: string): Result<string> {
  ...
  if (tags) input.tags = tags.split(",").map((s) => s.trim());
...
function cmdList(flags: Record<string, string>, sp: string): Result<string> {
  const storeResult = loadStore(sp);
  if (!storeResult.ok) return storeResult;
  ...
  const lines = sorted.map(
    (t) => `${t.id}  [${t.priority}] [${t.status}]  ${t.title}${t.project ? `  (${t.project})` : ""}`
  );
  return ok(lines.join("\n"));
}

function cmdShow(id: string | undefined, sp: string): Result<string> {
  if (!id) return err("Usage: cte show <id>");
...
function cmdDelete(id: string | undefined, sp: string): Result<string> {
  if (!id) return err("Usage: cte delete <id>");
...
function cmdGraph(sp: string): Result<string> {
  const storeResult = loadStore(sp);
  if (!storeResult.ok) return storeResult;
  ...
  function printTree(taskId: string, indent: string, isLast: boolean) {
  ...
  roots.forEach((root, i) => {
    ...
    const statusIcon =
      root.status === "completed" ? "\u2713" : root.status === "failed" ? "\u2717" : root.status === "in_progress" ? "\u2192" : "\u25CB";
...
function cmdRun(flags: Record<string, string>, sp: string): Result<string> {
  const storeResult = loadStore(sp);
  if (!storeResult.ok) return storeResult;
  ...
  if (plan.waves.length === 0) return ok("No tasks to execute.");
...
function cmdKanban(flags: Record<string, string>, sp: string): Result<string> {
  const storeResult = loadStore(sp);
  if (!storeResult.ok) return storeResult;
  ...
  try {
    ...
    return err(`Failed to write kanban: ${(e as Error).message}`);
  }
}

function cmdStats(sp: string): Result<string> {
  const storeResult = loadStore(sp);
  if (!storeResult.ok) return storeResult;
